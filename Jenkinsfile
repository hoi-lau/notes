pipeline {
    ...
    stages {
        stage('Build') {
            steps {
                echo '---building---'
                sh 'docker run --rm -v npm-repo:/root/.npm -v $(pwd):/root/app node:alpine sh -c "cd /root/app&&rm -f package-lock.json&&npm i --registry https://registry.npm.taobao.org&&npm run build"'
            }
        }
        stage('Test') {
            steps {
